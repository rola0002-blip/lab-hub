# LabHub — Mac Studio (Colima) Internet-Facing Server: Provisioning Runbook

LabHub runs on a Mac Studio under **Colima Docker**, fronted by the **Cloudflare Tunnel**
(`cloudflared`) over **HTTPS** at `https://labhub.<domain>`. The tunnel dials **outbound**
...
(→ `TUNNEL_TOKEN`). Add a **Public Hostname** `labhub.<domain>` → service `http://app:3000`
...
> before you finish setup. Without the gate, anyone who reaches `https://labhub.<domain>` first
...
> **APP_URL is exact.** It must equal `https://labhub.<domain>` — scheme + host, https, no
...
Install the two LaunchAgents (substitute your clone path), enable automatic login, disable sleep:
    ...
    REPO="$HOME/labhub"   # your clone path
    for a in stack backup; do
      sed "s#/ABSOLUTE/PATH/TO/lab-hub#$REPO#g" "scripts/macos/launchd/com.labhub.$a.plist" \
        > "$HOME/Library/LaunchAgents/com.labhub.$a.plist"
      launchctl load "$HOME/Library/LaunchAgents/com.labhub.$a.plist"
    done
    # System Settings → Users & Groups → Automatically log in (the operator account)
    sudo systemsetup -setcomputersleep Never

`com.labhub.stack` (RunAtLoad) starts Colima then the prod+tunnel stack; `com.labhub.backup`
...
> **launchd 03:00 timing is not a hard cron.** `com.labhub.backup` uses
...
> `com.labhub.stack` agent (RunAtLoad) must have brought Colima + the stack up first.
...
- [ ] `init-env.sh` writes a valid `.env` (secrets minted; VAPID minted **non-blank**; `SETUP_TOKEN` minted **and printed**; `APP_URL=https://labhub.<domain>`; `TUNNEL_TOKEN` set; `AUTH_TRUSTED_IP_HEADER=cf-connecting-ip`; `AUTH_RATE_LIMIT_MAX=10`; `APP_PORT=3000`).
- [ ] First `docker compose --profile prod --profile tunnel up -d --build` reaches the **setup wizard** at `https://labhub.<domain>` (tunnel healthy; TLS valid).
...
- [ ] **Server Action** end-to-end over the tunnel (e.g. create an invite on People) succeeds — no Origin/Host error. If it errors, add `experimental: { serverActions: { allowedOrigins: ['labhub.<domain>'] } }` to `next.config.ts` and re-verify (§4.4).
...
- [ ] The two `launchd` agents survive a **reboot**: after power-cycling, `colima` + the stack come up automatically and `https://labhub.<domain>` is reachable; the 03:00 backup agent fires and produces an artifact.
- [ ] **Security headers:** `curl -sI https://labhub.<domain>/` shows HSTS, `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy`, and the report-only CSP; a document fetch via `/uploads/...` shows `nosniff`.
- [ ] **Lighthouse "Installable" PWA audit** (Chrome DevTools) passes over HTTPS — the previously deferred check, now runnable.
- [ ] **Real phone push test:** on a phone, open `https://labhub.<domain>`, install the PWA, opt into notifications, trigger a notifying event (e.g. a mention/DM), and confirm a **push notification is delivered** — proving VAPID keys reached the container and delivery works.
...
catastrophe. Dumps are self-cleaning (`pg_dump --clean --if-exists`) — pipe straight in:
    ...
    # `<compose-project>_uploads`, e.g. `labhub_uploads` when cloned to `$HOME/labhub`; run
