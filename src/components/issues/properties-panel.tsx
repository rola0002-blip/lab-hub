      ...
      <Row label="Project">
        <span className="flex items-center gap-0.5">
          {issue.project
            ...
            : <span className="px-1.5 py-0.5 text-sm text-muted">No project</span>}
          {/* The change-project affordance is now its own control with its own
              accessible name — TRIGGER is reused so hover and focus-ring behaviour
              stay uniform with the other four rows. Guests get the link only. */}
          {canEdit && (
            <Menu label="Set project" buttonClassName={TRIGGER} button={<ChevronDown size={14} aria-hidden className="text-subtle" />}
              items={[
