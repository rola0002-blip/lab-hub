...
export function IssueListView({ issues, role, users, timezone, today, closedGrouped = false, empty = null }: {
  issues: IssueDto[]; role: Role; users: Opt[]; timezone: string; today: string; closedGrouped?: boolean; empty?: React.ReactNode
}) {
      ...
      {groups.map((g, gi) => (
        <div key={g.key}>
          ...
          {g.items.map((issue, j) => {
            ...
            return (
              <IssueRow key={issue.id} issue={issue} role={role} users={users} timezone={timezone} today={today} tabIndex={idx === active ? 0 : -1} onFocusIndex={() => setActive(idx)} />
            )
          })}
        </div>
      ))}
