...
export function BoardView({ initial, role }: { initial: IssueDto[]; role: Role }) {
  ...
  const [collapsed, setCollapsed] = useState<Set<IssueStatus>>(new Set(['CANCELED'])) // Canceled collapsed by default
      ...
      {ISSUE_STATUSES.map((status) => {
        ...
        return (
          <section key={status} aria-label={STATUS_LABEL[status]} data-col-status={status}
            ...
            <button type="button" onClick={() => { const n = new Set(collapsed); if (isCollapsed) n.delete(status); else n.add(status); setCollapsed(n) }}
              className="flex items-center gap-2 rounded-md px-1 py-0.5 text-xs font-semibold text-muted hover:bg-hover">
