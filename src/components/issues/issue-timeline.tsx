...
export function IssueTimeline({ entries, selfId, role, names }: { entries: TimelineEntry[]; selfId: string; role: Role; names: Map<string, string> }) {
  const now = new Date()
      ...
      {entries.map((e) => e.kind === 'activity' ? (
        ...
        <li key={e.id} className="flex gap-2">
          ...
          <div className="min-w-0 flex-1 rounded-lg border border-border bg-surface p-2">
            <div className="flex items-baseline gap-2">
              ...
              {!e.comment.deleted && (e.comment.author.id === selfId || role === 'admin') && (
                <Menu label="Comment actions" button={<MoreHorizontal size={15} aria-hidden />} items={[
                  ...
                  { label: 'Delete', danger: true, onSelect: () => start(() => deleteCommentAction(e.comment.id).then(() => {})) },
                ]} />
              )}
            </div>
            {e.comment.deleted ? <p className="text-sm italic text-subtle">comment deleted</p> : editing === e.comment.id ? (
              <div className="mt-1">
                ...
                <div className="mt-1 flex gap-2 text-xs">
                  <button onClick={() => start(() => editCommentAction(e.comment.id, draft).then(() => setEditing(null)))} className="rounded bg-accent px-2 py-0.5 font-medium text-accent-on focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring-focus)]">Save</button>
            ...
            ) : <p className="whitespace-pre-wrap break-words text-sm text-default">{renderTokens(e.comment.body, names, selfId)}</p>}
          </div>
        </li>
      ))}
    </ol>
