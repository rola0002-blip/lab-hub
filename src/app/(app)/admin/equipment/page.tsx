...
export default async function EquipmentAdminPage() {
  ...
  return (
    <div>
      <p className="text-sm font-medium text-gray-400">03 — Equipment</p>
      <div className="mt-1 flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Equipment</h1>
        <Link href="/admin/equipment/new" className="rounded-md bg-accent px-4 py-2 text-sm font-medium text-white">Add equipment</Link>
      </div>
      <ul className="mt-6 divide-y divide-gray-100 rounded-xl border border-gray-200">
        {equipment.map((e) => (
          <li key={e.id} className={`flex items-center justify-between p-3 text-sm ${e.status === 'RETIRED' ? 'opacity-50' : ''}`}>
            <span>{e.name} <span className="text-gray-500">· {e.location}</span>{e.status === 'RETIRED' && ' · retired'}</span>
