...
- One hand-written, additive database migration (`20260805000000_project_manual_order`) adds `Project.rank`, a base-62 fractional index stored `COLLATE "C"` with its own index. Existing projects are backfilled into that newest-first baseline, every statement is guarded so the migration is re-runnable, and nothing is renamed or dropped — rolling back to the previous release stays data-safe.
