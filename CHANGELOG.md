...
- A weekly update prompt: a background job runs on the organisation's chosen day and hour — new settings under Settings → Organisation, defaulting to Tuesday 16:00 in the org timezone — and sends one direct message per project that has not been updated this week, to the project lead, or to the people with open work in it, or to the admins if there is no one else. The prompt is latched per project per week, so a restart never double-pings. Each project's menu gains "Skip the next prompt", "Pause updates for 4 weeks", and "Resume update prompts".
