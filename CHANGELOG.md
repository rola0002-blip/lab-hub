...
### Added
- The instrument catalogue reads as what you can actually book. `/booking` now opens in up to three bands — Available to you, Needs certification, and Retired — each shown only when it holds something, so an instrument you are blocked from reserving no longer sits beside one you could book this afternoon. The split follows the booking rules themselves rather than a second guess about them: an instrument that needs approval is still available to you, because requesting approval is a booking you can make, and certification blocks everyone equally, so an admin who is not certified on an instrument lands under Needs certification with everybody else. Every card in that band now names the instrument's equipment managers, and so does the Policy panel on the instrument's own page — until now "Ask an equipment manager to certify you" named nobody, and for a guest, who cannot open the certifications page at all, that one red chip was the only sign anything was missing. An instrument with no managers assigned names none.
- A project can point at a folder in Files. Choose one in the new "Files folder" field when you create or edit a project and the project page lists that folder's contents below the header — file name, size, who uploaded it, when — each name opening or downloading the file, with "Open in Files →" for uploading anything new or renaming what is there. A project with no folder linked shows no section at all, so nothing changes for the projects that do not want one. Linking is a shortcut, not a permission: folders stay shared workspace-wide and readable by everyone including guests, as the field says in as many words, and the same folder can back more than one project.
- Project updates can be corrected and retracted. Whoever wrote an update can edit it — the wording and the health call, with no time limit — and the row carries "(edited)" afterwards. The author, or any admin, can delete one, and the feed keeps that row in place as an empty note rather than closing the gap, because a retracted update is part of a project's record too. From then on it stops counting as the project's latest: "updated N days ago", the health chip, the projects grid and the weekly prompt's digest all fall back to the update before it. The announcement already posted to `#lab-updates` stays as it was written — a line the lab has already read cannot honestly be unsent.
